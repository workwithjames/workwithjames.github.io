...
function staticAudit() {
  ...
  for (const file of htmlFiles) {
    ...
    if (!/<meta\b[^>]*name=["']description["']/i.test(source)) pushIssue(issues, 'warning', 'seo', location, 'Missing meta description');
    if (!/<link\b[^>]*rel=["']canonical["']/i.test(source)) pushIssue(issues, 'warning', 'seo', location, 'Missing canonical link');
    if ((source.match(/<h1\b/gi) || []).length !== 1) pushIssue(issues, 'warning', 'html', location, `Expected one h1, found ${(source.match(/<h1\b/gi) || []).length}`);
    ...
    for (const match of source.matchAll(/<a\b([^>]*)target=["']_blank["']([^>]*)>/gi)) {
      ...
      if (!relMatch || !/\bnoopener\b/i.test(relMatch[1])) pushIssue(issues, 'warning', 'security', location, 'target="_blank" link missing rel="noopener"', match[0].slice(0, 180));
    }
  }

  for (const file of jsFiles) {
    const source = fs.readFileSync(file, 'utf8');
    if (/window\.open\([^\n]+['_"]_blank['_"][^\n]*\)(?![\s\S]{0,40}noopener)/.test(source) && !source.includes("'noopener,noreferrer'")) {
  ...
  for (const xmlFile of ['sitemap.xml', 'feed.xml', 'image-sitemap.xml']) {
    ...
    if (fs.existsSync(full)) {
      ...
      if ((source.match(/<loc>/g) || []).some?.(() => false)) {}
    }
  }
...
function sitemapRoutes() {
  ...
  ['/contact/', '/buy-invest-dubai/', '/sell-dubai-property/', '/real-estate-marketing/', '/dubai-data/', '/abu-dhabi-data/', '/ajman-data/', '/blog/'].forEach((route) => routes.add(route));
  return [...routes].sort();
}
    ...
    for (const route of routes) {
      ...
      try {
        ...
        const result = await page.evaluate(() => {
          ...
          const blankLinks = [...document.querySelectorAll('a[target="_blank"]')].filter((a) => !/\bnoopener\b/i.test(a.rel)).map((a) => a.outerHTML.slice(0, 180));
          const unlabeled = [...document.querySelectorAll('input:not([type="hidden"]),select,textarea')].filter((field) => {
          ...
          const unnamedButtons = [...document.querySelectorAll('button')].filter((button) => !(button.textContent || '').trim() && !button.getAttribute('aria-label') && !button.getAttribute('aria-labelledby')).map((button) => button.outerHTML.slice(0, 180));
          const directNavOrder = (selector) => {
            const nav = document.querySelector(selector);
            if (!nav) return [];
            return [...nav.children].map((child) => child.matches('details') ? (child.querySelector(':scope > summary')?.textContent || '').replace(/[⌄⌃]/g, '').trim() : (child.textContent || '').trim()).filter(Boolean);
          };
          return {
            duplicateIds,
        ...
        if (!result.hasDescription) pushIssue(issues, 'warning', 'seo', location, 'Rendered page lacks meta description');
        if (!result.hasCanonical) pushIssue(issues, 'warning', 'seo', location, 'Rendered page lacks canonical link');
        if (result.horizontalOverflow) pushIssue(issues, 'error', 'layout', location, `Horizontal overflow by ${result.overflowBy}px`);
        for (const item of result.blankLinks) pushIssue(issues, 'warning', 'security', location, 'Rendered target="_blank" link missing noopener', item);
        ...
        const actual = viewport.name === 'mobile' ? result.mobileNavOrder : result.desktopNavOrder;
        ...
        if (viewport.name === 'mobile') {
          if (!result.mobileToggleVisible) pushIssue(issues, 'error', 'navigation', location, 'Mobile menu button is not visible');
          const toggle = page.locator('.mobile-menu-toggle');
          if (await toggle.count()) {
            await toggle.click();
            const menu = page.locator('#mobile-site-menu');
            if (!(await menu.isVisible())) pushIssue(issues, 'error', 'navigation', location, 'Mobile menu does not open');
