...
s=requests.Session();s.headers.update({'User-Agent':'Mozilla/5.0 (compatible; JamesRaviDataProbe/1.1)','Accept-Language':'en-US,en;q=0.9'})
result={'scripts':[]}
for url in urls:
 item={'url':url}
 try:
  ...
  item['urls']=sorted(set(re.findall(r'https?://[^"\'<>\\\s]+',text)))[:300]
  item['paths']=sorted(set(re.findall(r'["\'](/[^"\'<>\s]{3,200})["\']',text)))[:500]
  keys=['ajax','fetch(','transaction','total','mortgage','gift','sales','api.','gateway','getdaily','summary']
  ...
  for key in keys:
   ...
   while True:
    ...
    if pos<0:break
    snippets.append(text[max(0,pos-400):pos+1000])
    start=pos+len(key)
