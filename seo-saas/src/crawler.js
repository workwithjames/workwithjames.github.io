const BOT_UA = "JamesSEOAuditBot/0.2 (+https://seo.jamesrealty.uk)";
const MAX_PAGES = 25;
const MAX_HTML_BYTES = 1_500_000;
const FETCH_TIMEOUT_MS = 8_000;

export async function auditTarget(value, requestedPages = 15) {
  const parsed = normalizeTarget(value);
  if (!parsed.ok) throw new AuditInputError(parsed.error);

  const target = parsed.url;
  const maxPages = Math.max(1, Math.min(MAX_PAGES, Number.isFinite(Number(requestedPages)) ? Number(requestedPages) : 15));
  ...
  return {
    ...
    robots: { reachable: robots.reachable, status: robots.status, disallowCount: robots.disallow.length },
    score,
    issueCounts: counts,
    statusCounts,
    pages,
...
async function crawlSite(startUrl, maxPages, robots) {
  ...
  while (queue.length && pages.length < maxPages) {
    ...
    if (isRobotsDisallowed(current, robots)) {
      pages.push({
        url: key, status: 0, contentType: "blocked-by-robots", responseMs: 0, score: 100,
        title: "", metaDescription: "", h1: "", canonical: "", wordCount: 0,
        links: { internal: 0, external: 0 }, images: { total: 0, missingAlt: 0 },
        issues: [{ severity: "notice", code: "robots-blocked", message: "Blocked by robots.txt and not crawled" }],
      });
      continue;
    }

    const page = await inspectPage(current, startUrl.origin);
    ...
    for (const href of page.internalLinks) {
      ...
      try {
        ...
        if (!isHttpUrl(next) || shouldSkipPath(next) || !isPublicHostname(next.hostname)) continue;
...
async function inspectPage(url, expectedOrigin) {
  const startedAt = Date.now();
  let response;
  try {
    response = await fetchWithTimeout(url.href, {
      headers: { "user-agent": BOT_UA, accept: "text/html,application/xhtml+xml;q=0.9,*/*;q=0.1" },
      redirect: "manual",
    });
    ...
    while (response.status >= 300 && response.status < 400 && response.headers.get("location") && redirects < 5) {
      ...
      if (!isHttpUrl(next) || !isPublicHostname(next.hostname)) throw new Error("Unsafe redirect target blocked");
      if (next.origin !== expectedOrigin) throw new Error("Cross-origin redirect blocked during crawl");
      response = await fetchWithTimeout(next.href, {
        headers: { "user-agent": BOT_UA, accept: "text/html,application/xhtml+xml;q=0.9,*/*;q=0.1" },
        redirect: "manual",
      });
      url = next;
    ...
    return {
      internalLinks: [],
      publicData: {
        url: url.href, status: 0, contentType: "fetch-error", responseMs: Date.now() - startedAt, score: 0,
        title: "", metaDescription: "", h1: "", canonical: "", wordCount: 0,
        links: { internal: 0, external: 0 }, images: { total: 0, missingAlt: 0 },
  ...
  if (!contentType.toLowerCase().includes("text/html")) {
    ...
    return { internalLinks: [], publicData: {
      url: finalUrl, status, contentType, responseMs, score: issues.length ? 40 : 100,
      title: "", metaDescription: "", h1: "", canonical: "", wordCount: 0,
      links: { internal: 0, external: 0 }, images: { total: 0, missingAlt: 0 }, issues,
    }};
  }
  ...
  if (contentLength && contentLength > MAX_HTML_BYTES) {
    return { internalLinks: [], publicData: {
      url: finalUrl, status, contentType, responseMs, score: 60,
      title: "", metaDescription: "", h1: "", canonical: "", wordCount: 0,
      links: { internal: 0, external: 0 }, images: { total: 0, missingAlt: 0 },
  ...
  return { internalLinks: data.internalLinkHrefs, publicData: {
    url: finalUrl, status, contentType, responseMs, score,
    title: data.title, metaDescription: data.metaDescription, h1: data.h1,
    h1Count: data.h1Count, h2Count: data.h2Count, canonical: data.canonical,
    robots: data.robots, lang: data.lang, wordCount: data.wordCount, schemaCount: data.schemaCount,
    links: { internal: data.internalCount, external: data.externalCount },
    images: { total: data.imageCount, missingAlt: data.missingAlt }, issues,
  }};
}
...
function parseHtml(html, pageUrl) {
  ...
  return {
    title, metaDescription, canonical, robots, lang, viewport,
    h1: h1s[0] || "", h1Count: h1s.length, h2Count,
    imageCount: imageTags.length, missingAlt, schemaCount, wordCount,
    internalCount, externalCount, internalLinkHrefs,
  };
}
...
function calculatePageScore(issues) {
  let score = 100;
  for (const issue of issues) score -= issue.severity === "critical" ? 18 : issue.severity === "warning" ? 8 : 3;
...
async function readRobots(target) {
  ...
  try {
    const response = await fetchWithTimeout(robotsUrl.href, { headers: { "user-agent": BOT_UA }, redirect: "manual" });
...
function canonicalize(url) {
  ...
  next.hash = "";
  for (const key of [...next.searchParams.keys()]) if (/^(utm_|gclid$|fbclid$|msclkid$)/i.test(key)) next.searchParams.delete(key);
...
function isHttpUrl(url) { return url.protocol === "http:" || url.protocol === "https:"; }
...
function matchFirst(html, regex) { return (html.match(regex) || [])[1] || ""; }
...
function safeError(error) { return (error instanceof Error ? error.message : String(error || "Unknown error")).slice(0, 160); }
