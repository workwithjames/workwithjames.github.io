(function(){
  ...
  function esc(value){return text(value).replace(/[&<>"']/g,function(c){return {'&':'&amp;','<':'&lt;','>':'&gt;','"':'&quot',"'":'&#39;'}[c];});}
  ...
  function createSection(){
    ...
    section.innerHTML='<div class="section-heading market-heading"><div><p class="section-kicker">Yesterday, or latest completed day</p><h2 id="yesterday-transactions-title">Previous-day property transactions.</h2></div><p id="yesterday-transactions-date">Dubai date</p></div>'+
  ...
  function render(snapshot){
    var requested=snapshot.requested_date||expectedYesterday();var date=snapshot.target_date||requested;var rows=Array.isArray(snapshot.latest_transactions)?snapshot.latest_transactions:[];var areas=Array.isArray(snapshot.top_areas)?snapshot.top_areas:[];var fallback=!!snapshot.is_fallback||date!==requested;var stale=requested!==expectedYesterday();
    ...
    else if(fallback){status('No records were available for '+displayDate(requested)+'. Showing the latest completed day, '+displayDate(date)+'. Refreshed '+displayTimestamp(snapshot.generated_at)+'.','saved');}
    else if(stale){status('Showing the saved previous-day snapshot for '+displayDate(date)+'. A newer daily refresh is pending.','saved');}
    ...
    document.getElementById('yesterday-source-note').textContent='Source: DLD-derived transaction records delivered by DXBData. The server first requests '+displayDate(requested)+'. '+(fallback?'Because that date was not available, this panel shows the latest completed transaction date, '+displayDate(date)+'. ':'')+'The saved result remains accessible even when the external service is temporarily unavailable.';
  ...
  async function load(force){
    ...
    try{var response=await fetch(DATA_URL+'?v='+(force?Date.now():'2'),{cache:'no-store'});if(!response.ok){throw new Error('HTTP '+response.status);}render(await response.json());}
